...
std::size_t ActionRegistry::insert(ActionFactory const &action) {
  ...
  if (it != factories.end()) {
    throw ActionException(
        std::format("Action {} already exists in this registy", action.name));
  }
...
void ActionRegistry::remove(std::string const &name) {
  ...
  if (it == factories.end()) {
    throw ActionException(
        std::format("Action {} does not exists in this registy", name));
  }
...
ActionRegistry::lookupByWeightOffset(std::size_t offset) const {
  ...
  if (it == factories.end()) {
    throw ActionException(
        std::format("Weight offset {} is outside of this registy", offset));
  }
