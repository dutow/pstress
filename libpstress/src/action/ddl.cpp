...
Column randomColumn(ps_random &rand, bool forceSerial = false) {
  ...
  col.name = std::format("col{}", rand.random_number<std::size_t>());
...
std::string columnDefinition(Column const &col) {
  if (col.auto_increment) {
    ...
    return std::format("{} {}", col.name, "SERIAL");
  } else {
    std::string def =
        std::format("{} {}", col.name, rfl::enum_to_string(col.type));
    if (col.length > 0) {
      def += std::format("({})", col.length);
    }
    return def;
  }
}
...
void CreateTable::execute(Metadata &metaCtx, ps_random &rand,
  ...
  metaCtx.createTable([&](Metadata::Reservation &res) {
    ...
    table->name = std::format("foo{}", rand.random_number(1, 1000000)); // name;
    ...
    if (!pk_columns.empty()) {
      defs.push_back(std::format("PRIMARY KEY ({})",
                                 boost::algorithm::join(pk_columns, ", ")));
    }

    connection
        ->executeQuery(std::format("CREATE TABLE {} ({});", table->name,
...
void DropTable::execute(Metadata &metaCtx, ps_random &rand,
  ...
  metaCtx.dropTable(idx, [&](Metadata::Reservation &res) {
    if (!res.open())
      return;
    connection->executeQuery(std::format("DROP TABLE {};", res.table()->name))
        .maybeThrow();
    ...
    for (std::size_t idx = 0; idx < howManySubcommands; ++idx) {
      ...
      case AlterSubcommand::addColumn: {
        ...
        alterSubcommands.emplace_back(
            std::format("ADD COLUMN {}", columnDefinition(column)));
      ...
      case AlterSubcommand::dropColumn: {
        ...
        alterSubcommands.emplace_back(
            std::format("DROP COLUMN {}", table->columns[columnIndex].name));
